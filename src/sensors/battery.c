...
static int divider_setup(void)
{
    ...
    LOG_INF("Initializing the battery");
	...
	if (gcp->label) {
		...
		if (ddp->gpio == NULL) {
			LOG_ERR("Failed to get GPIO %s", log_strdup(gcp->label));
			return -ENOENT;
		}
		...
		if (rc != 0) {
			LOG_ERR("Failed to control feed %s.%u: %d",
				log_strdup(gcp->label), gcp->pin, rc);
			return rc;
		}
	}
...
int16_t sensor_get_battery_mV(void)
{
	if (!is_battery_init) {
		LOG_ERR("Battery is not initialized yet");
		return -1;
	}
	...
	if (battery_ok) {
		...
		if (rc == 0) {
			...
			if (dcp->output_ohm != 0) {
				batt_mv = val * (uint64_t)dcp->full_ohm / dcp->output_ohm;
			} else {
				batt_mv = val;
			}
		}
...
double sensor_get_battery_soc(int16_t batt_mV)
{
	...
	/* Iterate SoC table to the last point at or below the measured voltage. */
